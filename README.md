...
| Form | URL |
|------|-----|
| Subdomain (needs Ada wildcard DNS + TLS) | `https://{username}.reps.decisionsciencecorp.com/` |
| Path fallback (works now) | `https://reps.decisionsciencecorp.com/a/{username}/` |

Reserved host labels (`www`, `dashboard`, `api`, `join`, …) cannot be affiliate slugs — see `public/includes/affiliate_pages.php`. Stubs under `public/a/{slug}/` are generated by `php tools/sync_affiliate_page_stubs.php` (Empanada `/p/{slug}/` Option C pattern). Join from the page attributes `?rep={username}`.
