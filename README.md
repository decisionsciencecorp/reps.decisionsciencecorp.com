...
### Affiliate pages (Chuck ask)

Each active **sales** seat gets a public intro page (not just a code):

**Canonical:** `https://reps.decisionsciencecorp.com/a/{username}/`

Path stubs only (Empanada `/p/{slug}/` Option C) — **no subdomain DNS or nginx changes**. Reserved path segments (`www`, `dashboard`, `api`, `join`, `a`, …) cannot be affiliate slugs — see `public/includes/affiliate_pages.php`. Regenerator: `php tools/sync_affiliate_page_stubs.php`. Join from the page attributes `?rep={username}`.
